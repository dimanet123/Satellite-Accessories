import socket
import time
import struct
import numpy as np
import matplotlib.pyplot as plt
import sys
sys.path.insert(0, '../python_classes/')
from class_3dobject import Object3D, calculate_force, update_system

UDP_IP = "127.0.0.1"
UDP_PORT = 6501
sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

def send_udp_data(DAT):
    # Упаковываем массив данных в байты
    buf = struct.pack('<' + 'd' * len(DAT), *DAT)
    # Отправляем упакованные данные
    sock.sendto(buf, (UDP_IP, UDP_PORT))
    
object1 = Object3D(mass=1,x = 0)
object2 = Object3D(mass=1,x = 10)
object3 = Object3D(mass=1,x = 5, y = 8.66, z = 0)
object4 = Object3D(mass=1,x = 5, y = 2.89, z = 8.16)
objects = [object1, object2, object3, object4]  # Создаем два объекта
connections = [(0, 1),(1,2),(0,2),(3,0),(3,1),(3,2)]  # Связываем их
spring_constants = [10,10,10,10,10,10]  # Коэффициент упругости
rest_lengths = [9,9,9,9,9,9]  # Ненагруженная длина пружины
dt = 0.001/100  # Шаг времени
time_animation = 0.05
time_div = int(time_animation/dt)

def create_cube(width, height, depth, mass=1, spring_constant=10, rest_length=10):
    objects = []
    connections = []
    spring_constants = []
    rest_lengths = []

    # Генерируем грузы
    for x in [0, width]:
        for y in [0, height]:
            for z in [0, depth]:
                objects.append(Object3D(mass=mass, x=x, y=y, z=z))

    # Генерируем связи
    for i, obj_i in enumerate(objects):
        for j, obj_j in enumerate(objects):
            if i < j:  # Чтобы не создавать дубликаты связей
                distance = np.sqrt((obj_i.x - obj_j.x)**2 + (obj_i.y - obj_j.y)**2 + (obj_i.z - obj_j.z)**2)
                if distance <= np.sqrt(width**2 + height**2 + depth**2):  # Соединяем только соседние грузы
                    connections.append((i, j))
                    spring_constants.append(spring_constant)
                    rest_lengths.append(rest_length)

    return objects, connections, spring_constants, rest_lengths

objects, connections, spring_constants, rest_lengths = create_cube(width=3, height=3, depth=3, mass=1, spring_constant=10, rest_length=10)

# Обновляем систему


# Выводим новые положения и скорости объектов
for obj in objects:
    print(obj)

while True:
    start_time = time.time()
    
    for i in range(time_div):
        update_system(objects, connections, spring_constants, rest_lengths, dt, 0)
        
    # Ваш код здесь
    end_time = time.time()
    time_dif = end_time - start_time
    print(f'{time_div},{time_dif}')
    DAT = [0]
    for obj in objects:
        DAT += obj.form_udp()
    
    send_udp_data(DAT)
    # for obj in objects:
    #     print(obj)

    
    # time.sleep(time_animation)